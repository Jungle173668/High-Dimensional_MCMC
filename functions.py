...
def get_G(N, idx):
    ...
    Inputs:
        N - Length of vector to subsample (length of u)
...
def predict_t(samples):
    ...
    return MC_sum / N  # TODO: Return p(t=1|samples)


###--- Density functions ---###

def log_prior(u, K, K_inverse):
    # Return observation likelihood p(u), TODO:
    N = len(u)
...
def log_continuous_likelihood(u, v, G):
    # Return observation likelihood p(v|u), TODO:
    M = len(v)
...
def grw(log_target, u0, data, K, G, n_iters, beta):
    ...
    for i in range(n_iters):
        z = np.random.randn(N)  # Random points
        u_new = u_prev + beta * Kc @ z  # TODO: Propose new sample - use prior covariance, scaled by beta; Kc@z is under the distribution of N(0,K)
...
def pcn(log_likelihood, u0, y, K, G, n_iters, beta):
    ...
    for i in range(n_iters):
        z = np.random.randn(N)
        u_new = (np.sqrt(1-beta**2) * u_prev) + (beta * (Kc @ z)) # TODO: Propose new sample using pCN proposal
